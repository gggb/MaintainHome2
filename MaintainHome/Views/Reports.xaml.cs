...
namespace MaintainHome.Views
{
    ...
    {
        public string NoScheduledTasksMessage { get; set; }
        ...
        public Reports()
        {
            ...
            NoScheduledTasksMessage = string.Empty;
        ...
        private async Task LoadReports(string period)
        {
            try
            {
                ...
                {
                    NoScheduledTasksLabel.IsVisible = true; // Show the message
                    TaskScheduleCollectionView.IsVisible = false; // Hide the collection view
                    return; // Exit the method early if there are no scheduled tasks
                }
                else
                {
                    // Add the main report
                    ...
                    foreach (var task in report.Tasks)
                    {
                        ...
                        Debug.WriteLine($"Added task report: {taskReport.Title} with due date {taskReport.DueDate},Ctreation date: {task.CreatedDate} and Modified Date: {task.ModifiedDate}  ");
                    }
                }
